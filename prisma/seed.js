...
async function main() {
  ...
  const plots = [];
  const plotNumbers = ['A-101', 'A-102', 'A-103', 'B-201', 'B-202', 'B-203', 'C-301', 'C-302', 'C-303', 'D-401', 'D-402', 'D-403', 'E-501', 'E-502', 'F-601', 'F-602', 'G-701', 'G-702', 'H-801', 'H-802'];
  ...
  const adminUser = await prisma.users.create({
    data: {
      ...
      status: 'approved',
      blood_group: 'O+',
      password_hash: '$2b$10$dummy.hash.for.demo.purposes', // In real app, hash properly
    }
  });
  ...
  const fees = [];
  for (const user of users.slice(2)) { // Skip admin and owner
    for (let month = 0; month < 6; month++) {
      const fee = await prisma.fees.create({
        data: {
          user_id: user.user_id,
          month: new Date(2024, month, 1),
          amount_due: faker.number.float({ min: 5000, max: 15000, fractionDigits: 2 }),
          status: faker.helpers.arrayElement(['pending', 'paid', 'partial', 'overdue']),
        }
      });
      fees.push(fee);
    }
  ...
  for (const fee of fees.filter(f => f.status === 'paid' || f.status === 'partial')) {
    await prisma.feePayments.create({
      data: {
        fee_id: fee.fee_id,
        amount_paid: fee.status === 'partial' 
          ? fee.amount_due * 0.5 
          : fee.amount_due,
        payment_date: faker.date.recent({ days: 30 }),
        payment_method: faker.helpers.arrayElement(['cash', 'bkash', 'sslcommerz', 'bank_transfer']),
  ...
  for (const staff of staffUsers) {
    const assignment = await prisma.payrollAssignments.create({
      data: {
        ...
        role: faker.helpers.arrayElement(['security', 'maintenance', 'cleaning', 'gardener']),
        base_amount: faker.number.float({ min: 15000, max: 35000, fractionDigits: 2 }),
        frequency: 'monthly',
  ...
  for (const type of contributionTypes) {
    const contribution = await prisma.contributions.create({
      data: {
        ...
        collected_amount: faker.number.float({ min: 0, max: type.target * 0.7, fractionDigits: 2 }),
  ...
  for (let i = 0; i < 15; i++) {
    await prisma.expenses.create({
      data: {
        category: faker.helpers.arrayElement(['maintenance', 'utilities', 'security', 'cleaning', 'repairs']),
        description: faker.lorem.sentence(),
        total_amount: faker.number.float({ min: 1000, max: 50000, fractionDigits: 2 }),
        paid_by: faker.helpers.arrayElement(['cash', 'bkash', 'sslcommerz']),
        payment_date: faker.date.recent({ days: 60 }),
        approved_by: faker.helpers.arrayElement([adminUser.user_id, ownerUser.user_id]),
